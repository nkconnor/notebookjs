...
class ReactJupyterInput extends React.Component {

    render() {
        let {raw, cell} = this.props;

        let lang = cell.raw.language ||
            cell.worksheet.notebook.language ||
            cell.worksheet.notebook.language_info.name;

        let holder = (
            <div>
                <input data-prompt-number={cell.number}  />
                <pre>
                    <code data-language={lang} className={`lang-${lang}`}>
                        {`${raw}`}
                    </code>
                </pre>
            </div>
